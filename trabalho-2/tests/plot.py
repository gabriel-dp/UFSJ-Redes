...
for exec_num in data['server'].unique():
    ...
    plt.plot(exec_data['users'], exec_data['total'] - exec_data['fails'], marker='o', label=f'Servidor {exec_num}')
...
for exec_num in data['server'].unique():
    exec_data = data[data['server'] == exec_num]
    plt.plot(exec_data['users'], exec_data['avg_time'], marker='o', label=f'Servidor {exec_num}')
